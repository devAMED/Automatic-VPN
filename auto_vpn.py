...
def ensure_vpnbook_auth():
    ...
    log("VPNBook password not found in auth.txt")
    log("Enter the current VPNBook password from the official VPNBook website.")
    password = getpass.getpass("VPNBook password: ").strip()
    if not password:
        log("Password cannot be empty.")
        sys.exit(1)

    AUTH_FILE.write_text(f"vpnbook\n{password}\n", encoding="utf-8")
    os.chmod(AUTH_FILE, 0o600)
    log("Saved VPNBook credentials to auth.txt")


def download_vpnbook_configs():
    ...
    try:
        ...
        log(f"VPNBook download error: {error}")
        return 0

    hostnames = sorted(set(re.findall(r"\b[a-z]{2}\d+\.vpnbook\.com\b", response.text)))
    if not hostnames:
